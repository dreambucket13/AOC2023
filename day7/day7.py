    ...
    def __init__(self, handStr):
        ...
        for card in handStr:
            match card:
                ...
                case 'A'  : 
                    self.cardCounts[14] += 1
                    self.primaryRank += 14 * pow(10,secondaryRankPower)   
                case _  : 
                    pass
        ...
        def assignPrimaryRank(self):
            if self.fiveOfAKindCard is not None:
                self.primaryRank += FIVE_OF_A_KIND
                return

            if self.fourOfAKindCard is not None:
                self.primaryRank += FOUR_OF_A_KIND
                return
            
            if self.threeOfAKindCard is not None and len(self.twoOfAKindCards) == 1:
                self.primaryRank += FULL_HOUSE
                return      

            if self.threeOfAKindCard is not None:
                self.primaryRank += THREE_OF_A_KIND
                return      
            
            if len(self.twoOfAKindCards) == 2:
                self.primaryRank += TWO_PAIR
                return  
            
            if len(self.twoOfAKindCards) == 1:
...
def main():
    ...
    for (index, game) in enumerate(gameData):
        #print(f'hand: {game[0].handString}, rank: {game[0].primaryRank}')
